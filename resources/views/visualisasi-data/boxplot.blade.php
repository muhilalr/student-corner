<x-layout-web>
  <div class="min-h-screen bg-[#EEF0F2]">
    ...
    <div class="bg-blue-900 text-white py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold">Box Plot</h1>
        <p class="mt-2 text-blue-100">
          Visualisasikan sebaran data numerik Anda dengan box plot (min, Q1, median, Q3, max).
        </p>
      </div>
    </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div id="columnSelection" class="mb-6 hidden">
              ...
              <select id="columnSelect"
                ...
                <option value="">Pilih kolom untuk box plot...</option>
              </select>
            </div>
            ...
            <button id="generateBtn"
              ...
              <span id="generateBtnText">Generate Box Plot</span>
            ...
            <div id="emptyState" class="text-center py-12">
              <div class="text-gray-400 mb-4">
                <svg class="mx-auto h-16 w-16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 12h16M12 4v16" />
                </svg>
              </div>
              <h3 class="text-lg font-medium text-gray-900 mb-2">Belum ada data</h3>
              <p class="text-gray-500">Upload file data terlebih dahulu untuk membuat box plot</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      generateBtn.addEventListener('click', generateBoxPlot);
    ...
    function handleFileUpload(event) {
      ...
      fetch('{{ route('visualisasi.boxplot.upload') }}', {
          method: 'POST',
          body: formData,
          headers: {
        ...
        .then(data => {
          if (data.success) {
            ...
            uploadedColumnTypes = data.columnTypes || {};
    ...
    function populateColumnSelect(columns, numericColumns = [], columnTypes = {}) {
      const select = document.getElementById('columnSelect');
      select.innerHTML = '<option value="">Pilih kolom untuk box plot...</option>';

      if (numericColumns && numericColumns.length) {
        const groupNum = document.createElement('optgroup');
        groupNum.label = 'Numeric Columns (Recommended)';
        numericColumns.forEach(col => {
          const opt = document.createElement('option');
          opt.value = col;
          opt.textContent = `${col} (numeric)`;
          groupNum.appendChild(opt);
        });
        select.appendChild(groupNum);

        const others = columns.filter(c => !numericColumns.includes(c));
        if (others.length) {
          const groupOth = document.createElement('optgroup');
          groupOth.label = 'Other Columns';
          others.forEach(col => {
            const opt = document.createElement('option');
            opt.value = col;
            opt.textContent = `${col} (${columnTypes[col] || 'text'})`;
            groupOth.appendChild(opt);
          });
          select.appendChild(groupOth);
        }
      } else {
        columns.forEach(col => {
          const opt = document.createElement('option');
    ...
    function validateColumnIsNumeric(column) {
      const t = uploadedColumnTypes?.[column];
      if (t && t !== 'numeric') {
        showToast(`Kolom "${column}" bukan numerik`);
        return false;
      }
      return true;
    }
    ...
    function generateBoxPlot() {
      ...
      if (!validateColumnIsNumeric(column)) return;
      ...
      fetch('{{ route('visualisasi.boxplot.generate') }}', {
          method: 'POST',
          headers: {
        ...
        .then(data => {
          ...
          if (data.success) {
            renderBoxPlot(data.series, column);
            document.getElementById('emptyState').classList.add('hidden');
            document.getElementById('chartContainer').classList.remove('hidden');
          } else {
            showToast(data.error || 'Gagal membuat box plot');
          }
        })
        .catch(() => showToast('Gagal membuat box plot'));
    }

    function renderBoxPlot(seriesData, columnName) {
      const options = {
        series: [{
          type: 'boxPlot',
          data: seriesData // [{x:'All Data', y:[min,q1,median,q3,max]}]
        }],
        chart: {
          type: 'boxPlot',
          height: 400,
        ...
        tooltip: {
          ...
          custom: function({
            ...
            return `<div class="px-2 py-1">
                      <div><strong>Min:</strong> ${y[0]}</div>
                      <div><strong>Q1 :</strong> ${y[1]}</div>
                      <div><strong>Median:</strong> ${y[2]}</div>
                      <div><strong>Q3 :</strong> ${y[3]}</div>
                      <div><strong>Max:</strong> ${y[4]}</div>
                    </div>`;
          }
        }
      };
      ...
      currentChart = new ApexCharts(document.querySelector("#boxplot-chart"), options);
      currentChart.render();
    }
